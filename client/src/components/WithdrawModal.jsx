...
const WithdrawModal = ({onClose}) => {
    ...
    const handleSubmission = async (e) => {
        e.preventDefault();
        try {
            // check if theres at least one field
            if(account.length === 0) {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex
    items-center justify-center sm:p-4'>
    ...
    {/**Form */}
    <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4
    p-4 overflow-y-scroll'>
        <div className='grid grid-cols-[2fr_3fr_4fr] items-center gap-2'>
            Amount <input onChange={(e) => setAmount(e.target.value)} 
            value={amount} type="number" className='w-full px-2 py-1.5 text-sm border
            border-gray-300 rounded outline-indigo-400' required/>
        </div>
        {account.map((field, index)=>(
            <div key={index} className='grid grid-cols-[2fr_3fr_4fr] items-center gap-2'>
