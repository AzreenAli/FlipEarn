...
const CredentialSubmission = ({ onClose, listing }) => {
    ...
    const handleSubmission = async (e) => {
        e.preventDefault();
        try {
            // check if theres atleast one field
            if(credential.length === 0){
                return toast.error("Please add atleast one field");
            }

            // check all fields are failed
  ...
  return (
    ...
    flex items-center justify-center sm:p-4'>
      <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg max-h-[90vh]
      flex flex-col'>
        {/* Header */}
        <div className='bg-linear-to-r from-indigo-600 to-indigo-400 text-white p-4
        sm:eounded-t-lg flex items-center justify-between'>
        ...
        {/* Form */}
        <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4'>
            ...
            <div className='flex items-center gap-2'>
                <input type="text" value={newField} placeholder="field name..." onChange={(e) =>setNewField(e.target.value)} 
                className='outline-none border-b border-gray-200'/>
                <button type='button' onClick={handleAddField} className='flex items-center gap-2 text-sm text-gray-600
                hover:text-gray-700 cursor-pointer'>
                    <CirclePlus className='w-5 h-5'/></button>
            </div>
            {/**Submit Button */}
            <button type='submit' className='bg-indigo-600 hover:bg-indigo-700 text-white
            px-6 py-2 mt-4 rounded-md'>
                Submit
            </button>
        </form>
      </div>
